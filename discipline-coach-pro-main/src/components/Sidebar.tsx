...
import {
  ...
  Calendar as CalendarIcon, MessageCircle, Trophy,
  ...
  return (
    <aside className={`w-64 h-screen sticky top-0 flex flex-col border-r overflow-hidden ${sidebarBg}`}>
      ...
      <nav className="flex flex-col gap-1 p-4 flex-1 overflow-y-auto">
        <p className={`text-xs font-semibold uppercase tracking-wider px-4 mb-2 ${menuLabel}`}>
          Navigation
        </p>
        ...
        {/* Profile clearly accessible with avatar and user details */}
        <Link
          to="/profile"
          ...
          title="Open Profile"
        >
          <div className="flex items-center justify-center h-8 w-8 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 font-bold text-xs flex-shrink-0">
            {avatarLetter}
          </div>
          <div className="overflow-hidden flex-1 min-w-0">
            <div className="flex items-center justify-between">
              <span className={`text-xs font-bold ${profileName}`}>Profile</span>
              <span className="text-[10px] text-emerald-500 font-medium">Edit →</span>
            </div>
            <p className={`text-[11px] truncate ${profileEmail}`}>
              {user.name ? `${user.name}` : (user.email || "Manage Account")}
            </p>
          </div>
        </Link>
        ...
        <button
          ...
          className={`flex items-center gap-3 w-full px-4 py-2.5 rounded-xl transition-all duration-200 text-xs font-medium mt-1 ${logoutClass}`}
        >
